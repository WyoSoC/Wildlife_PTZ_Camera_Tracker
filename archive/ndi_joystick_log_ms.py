...
# =====================================================
CAMERA_NAME = "bird"
JOYSTICK_MATCH = "dualsense"   # keyword to match controller name
PAN_SPEED_SCALE = 0.8
...
def find_joystick(match: str):
    ...
    for i in range(count):
        js_tmp = pygame.joystick.Joystick(i)
        name = js_tmp.get_name()
        print(f"[{i}] {name}")
        if match.lower() in name.lower():
            found_index = i
...
def main():
    ...
    if SHOW_VIDEO:
        cv2.namedWindow("NDI Preview", cv2.WINDOW_NORMAL)
