...
export default defineConfig({
  ...
  server: {
    ...
    // Override with: BACKEND_PORT=8080 npm run dev
    proxy: {
      '/api': `http://localhost:${process.env.BACKEND_PORT ?? 9090}`,
      '/ws':  { target: `ws://localhost:${process.env.BACKEND_PORT ?? 9090}`, ws: true },
    },
  },
